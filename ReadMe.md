...
重复事件会按事件 ID 去重，乱序或过期事件不会覆盖较新的会话状态。长时间没有新 Hook 的运行中或等待批准任务会被标记为“异常 / 可能失联”，而不是继续伪装成正常运行。
...
用户点击展开任务页区块、活动托盘卡片或完成提示后，Atoll 通过 macOS `NSWorkspace` 激活已安装的 Codex App，并打开对应深链。具体行为是：
...
集成使用本地 Codex Hooks 和本地状态文件；不解析 transcript，不上传 prompt、源码或终端输出，也不启动用于 Atoll 集成的网络服务。关闭任务内容摘要后，展示层会主动去除提示、批准和结果文本，只保留项目名、状态和时间信息。
